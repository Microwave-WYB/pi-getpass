...
const JS = `(function(){"use strict";var f=document.getElementById("secret-form"),i=document.getElementById("secret"),b=document.getElementById("submit-btn"),s=document.getElementById("status"),h=document.getElementById("ttl-hint"),T=parseInt(document.body.dataset.ttl||"90",10),l=T;h.textContent="⏱ 请求方 "+l+"s 内未收到自动作废";var t=setInterval(function(){l-=1;h.textContent="⏱ 请求方 "+l+"s 内未收到自动作废";if(l<=0){clearInterval(t);h.textContent="⌛ 已过期，此页不再接受提交。";f.hidden=true}},1000);f.addEventListener("submit",function(e){e.preventDefault();var v=i.value;if(!v)return;b.disabled=true;b.textContent="提交中…";s.hidden=false;s.textContent="正在提交…";fetch(".",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({secret:v})}).then(function(r){if(r.ok){s.textContent="✅ 已收到，可关闭此页。";s.className="hint ok";f.hidden=true;clearInterval(t)}else{s.textContent="⚠️ 提交失败（HTTP "+r.status+"），请重试。";s.className="hint err";b.disabled=false;b.textContent="提交"}}).catch(function(){s.textContent="⚠️ 网络错误，请重试。";s.className="hint err";b.disabled=false;b.textContent="提交"});i.value=""})})();`;

function html(): string {
  const esc = PROMPT.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>");
  return `<!doctype html><html lang="zh"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>🔐 秘密输入</title><style>${CSS}</style></head><body data-ttl="${TTL}"><main><h1>🔐 秘密输入</h1><p class="prompt">${esc}</p><form id="secret-form" autocomplete="off"><input type="password" id="secret" autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false" placeholder="粘贴或输入秘密…" required><button type="submit" id="submit-btn">提交</button></form><p id="status" class="hint" hidden></p><p class="hint" id="ttl-hint"></p></main><script>${JS}<\/script></body></html>`;
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && safeEqual(url, `/${TOKEN}`)) {
    ...
    req.on("end", () => {
      ...
      SECRET = val;
      writeHead(res, 200, "text/html; charset=utf-8", "<!doctype html><meta charset=utf-8><title>ok</title><body><p>✅ 已收到，可关闭此页。</p></body>");
      server.close();
    });
    return;
  }

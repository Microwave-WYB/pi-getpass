...
Each request is isolated. Duplicate submissions, expiry, cancellation, and session shutdown close the server and remove temporary 0600 artifacts. The server binds only to Tailscale IPv4 (`100.64.0.0/10`) or Tailscale IPv6 ULA (`fd7a:115c:a1e0::/48`); loopback is accepted only for explicit local tests. The ready notification uses Pi's registration-time `sendUserMessage(..., { deliverAs: "followUp", expandPromptTemplates: false })`, so it remains agent-visible after tool settlement; it contains only the opaque request ID, `ready`, and the bounded status/consume instruction. Terminal request metadata is retained only within a bounded 128-request/15-minute window. The secret is never printed or included in chat/history, tool output, logs, intercom messages, or command text. Smoke test: `npx tsx tests/smoke-web.ts`.
